...
public class SnackAttackScript : MonoBehaviour
{
    ...
    private bool isPlayingSequenceAnimation;
    private bool isPlayingAutosolvedSequenceAnimation;
    private bool TwitchPlaysActive;
    private bool twitchMode;
    ...
    {
        isPlayingSequenceAnimation = true;
    ...
    IEnumerator LosingSound()
    {
        ...
        if (!isAutosolving)
            Module.HandleStrike();
        if (isPlayingAutosolvedSequenceAnimation)
        {
            string errorMessage = "Something has gone wrong in Snack Attack's autosolver. Please contact either Quinn Wuest (the module's autosolver developer) or BigCrunch (the module's main developer) and send a report with a log file.";
            Debug.LogFormat("[Snack Attack #{0}] {1}", moduleId, errorMessage);
            if (twitchMode)
                tpAPI["ircConnectionSendMessage"] = errorMessage;
            ModuleSolved = true;
            Module.HandlePass();
        ...
        PacmanColor = 0;
        AbleToCycle = true;
        isPlayingSequenceAnimation = false;
        StartUp();
    }
    ...
    void GenerateAbsolutelyEverything()
    {
        ...
        Debug.LogFormat("[Snack Attack #{0}] Color Sequence: {1}", moduleId, TheColorNumber.Select(i => new[] { "Red", "Green", "Blue" }[i]).ToArray().Join(", "));
    }
...
#pragma warning restore 414
    ...
    {
        isAutosolving = true;
        while (isPlayingSequenceAnimation)
            yield return true;
        if (!AbleToInput)
        {
            if (!AbleToPressNext)
                MainSelect.OnInteract();
            while (!AbleToPressNext)
                yield return true;
            PlayButton.OnInteract();
        }
        ...
        for (int i = 0; i < Position.Count; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                if (curPacManCol != TheColorNumber[i])
                {
                    VarietyOfOptions[3].OnInteract();
                    while ("RGB".IndexOf(RGB.text) != TheColorNumber[i])
                    {
                        VarietyOfOptions[3].OnInteract();
                        yield return new WaitForSecondsRealtime(0.05f);
                    }
                    curPacManCol = TheColorNumber[i];
                }
                else if (curPacManPos > Position[i])
                {
                    curPacManPos--;
                    VarietyOfOptions[0].OnInteract();
                    yield return new WaitForSeconds(0.05f);
        ...
        isPlayingAutosolvedSequenceAnimation = true;
        while (!ModuleSolved)
            yield return true;
    }
}